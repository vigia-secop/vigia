# VERIFICAR EL CAMBIO DE FORMATO. Solo lectura: deja verificar-formato.txt.
...
Write-Host ""
Write-Host "  EL CAMBIO DE HOY: FORMATO O CIFRAS" -ForegroundColor White
Write-Host "  Unos minutos." -ForegroundColor DarkGray
& $PSQL -h localhost -U vigia -d vigia -q -f "verificar-formato.sql" -o "verificar-formato.txt" 2>&1 | Sin-Rojo
if (Test-Path "verificar-formato.txt") {
    $b = (Get-Item "verificar-formato.txt").Length
    Write-Host "  Listo: verificar-formato.txt ($b bytes)" -ForegroundColor Green
}
Write-Host ""
