# QUE CAMBIO EN LA FUENTE HOY. Solo lectura: deja que-cambio-hoy.txt.
...
Write-Host ""
Write-Host "  QUE CAMPOS CAMBIARON ENTRE AYER Y HOY" -ForegroundColor White
Write-Host "  Unos minutos." -ForegroundColor DarkGray
& $PSQL -h localhost -U vigia -d vigia -q -f "que-cambio-hoy.sql" -o "que-cambio-hoy.txt" 2>&1 | Sin-Rojo
if (Test-Path "que-cambio-hoy.txt") {
    $b = (Get-Item "que-cambio-hoy.txt").Length
    Write-Host "  Listo: que-cambio-hoy.txt ($b bytes)" -ForegroundColor Green
}
Write-Host ""
