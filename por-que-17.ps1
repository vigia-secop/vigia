# VERIFICAR EL CAMBIO DE FORMATO. Solo lectura: deja por-que-17.txt.
...
Write-Host ""
Write-Host "  POR QUE HOY SOLO ENTRARON 17 CONTRATOS" -ForegroundColor White
Write-Host "  Unos minutos." -ForegroundColor DarkGray
& $PSQL -h localhost -U vigia -d vigia -q -f "por-que-17.sql" -o "por-que-17.txt" 2>&1 | Sin-Rojo
if (Test-Path "por-que-17.txt") {
    $b = (Get-Item "por-que-17.txt").Length
    Write-Host "  Listo: por-que-17.txt ($b bytes)" -ForegroundColor Green
}
Write-Host ""
